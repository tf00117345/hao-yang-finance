...
export function CompanyLabelsPrint({ open, onClose, companyIds }: CompanyLabelsPrintProps) {
	...
	return (
		<Dialog id="company-labels-print-dialog" open={open} onClose={onClose} maxWidth="lg" fullWidth>
			<DialogTitle id="company-labels-print-dialog-title">列印公司貼紙（A4 橫向 2x7）</DialogTitle>
			<DialogContent id="company-labels-print-dialog-content">
				{/* 僅限此區塊參與列印 */}
				<Box id="company-labels-print" sx={{ p: 0 }}>
									...
									{page.map((c) => (
										<Box key={c.id} className="label">
											<Box className="label-content">
												<Typography className="company-name">{c.name || '\u00A0'}</Typography>
